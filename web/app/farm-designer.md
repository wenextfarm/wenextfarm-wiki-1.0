...
![farm designer with tools panel open](_images/farm_designer_with_tools_panel_open.png =700px)
...
![soil interpolation](_images/soil_interpolation.png =700px)
...
> **ℹ️ Note:** Once a plant is created in the app, the spread value will not be updated if the plant's spread value changes in OpenFarm.
...
![spread curve](_images/spread_curve.png =700px)
...
> **ℹ️ Note:** See [camera calibration](photos/camera-calibration.md) if photos are not positioned, scaled, or rotated correctly.
...
![map z-axis display](_images/map_z_display.png =700px)

# Map settings

![map settings](_images/map_settings.png =700px)
...
> **⚠️ Note:** When enabled, **DYNAMIC MAP SIZE** will override **MAP SIZE** values.

> **ℹ️ Note:** **DYNAMIC MAP SIZE** only affects the display of the virtual garden map in the app. This setting will not affect the area in which FarmBot is allowed to move in. To change where FarmBot is allowed to move, see the settings available in the Axes section.
...
> **ℹ️ Note:** **MAP SIZE** only affects the display of the virtual garden map in the app. This setting will not affect the area in which FarmBot is allowed to move in. To change where FarmBot is allowed to move, see the settings available in the Axes section.
...
> **ℹ️ Note:** **ROTATE MAP** only affects the display of the virtual garden map in the app. This setting will not affect the coordinate system of the FarmBot.

## Map origin

Select a map origin by clicking on one of the four quadrants to adjust the virtual garden map to your viewing angle.

> **ℹ️ Note:** **MAP ORIGIN** only affects the display of the virtual garden map in the app. This setting will not affect the coordinate system of the FarmBot.
...
![farm designer move mode](_images/farm_designer_move_mode.png =700px)
...
![farm designer select mode](_images/farm_designer_select_mode.png =700px)
...
![profile viewer choose a location](_images/profile_viewer_choose_a_location.png =700px)
...
![profile viewer](_images/profile_viewer.png =700px)

![profile viewer (x-axis profile)](_images/profile_viewer_x.png =700px)
